...
int main() {
	CImg<unsigned char> testImg("..\\DataSet\\IMG5.jpg");
	int w = testImg.width();
	int h = testImg.height();
	int z = testImg.depth();
	...
	img_NMSppress.save_bmp("..\\Result\\result.bmp");
	/*-------------Hytheresis thresholding-----------------*/
	CImg<double> img_hytheshold(w,h,z,1);
	img_hytheshold = HysteresisThreshd(img_NMSppress, 130, 60);
	//img_hytheshold.display("edge");


	/*------------ Hough transfer and draw vertex--------------------*/
	CImg<double> HoughImg = HoughTransf(img_hytheshold, 0.4);
