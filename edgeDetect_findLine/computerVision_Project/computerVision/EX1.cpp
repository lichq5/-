...
int main() { 
	CImg<unsigned char> testImg("b8.bmp");
#if DEBUG
	CImgDisplay main_disp(testImg, "原图");
	/*---------旋转---------------*/
	/*CImgDisplay rotate_disp(rotate_img(testImg, -60), "二次线性内插旋转结果");
	CImgDisplay rotate_disp2(rotate_img2(testImg, -60), "最邻近内插旋转结果");
	CImgDisplay rotate_disp3(testImg.rotate(60, 1), "调用旋转库函数结果");
	*/
	rotate_img(testImg, -60).display("二次线性内插旋转结果");
	rotate_img2(testImg, -60).display("最邻近内插旋转结果");
	testImg.rotate(60, 1).display("调用旋转库函数结果");

	/*---------缩放---------------*/
	CImg<unsigned char> testImg2("2.bmp");
	testImg2.display("原图");
	CImgDisplay disp2(imresize(testImg2, .5), "二次线性内插缩放结果");
	CImgDisplay disp3(imresize(testImg2, 2), "二次线性内插缩放结果");
#endif
	/*---------画图---------------*/
	CImg<unsigned char> graphics(1024, 1024, 1, 3);
	graphics.fill(0);
	graphics = draw_rectangle(graphics, 300, 300, 100, 100);
	graphics = draw_circular(graphics, 100, 450, 450);
	graphics = draw_triangle(graphics, 300, 150, 600, 500, 60);
	graphics.display("graphics image");
	system("pause");
    return 0;
} 
